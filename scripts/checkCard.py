...
def main():
    ...
    """
    with open('/opt/magstripe/data/chrMap.json') as chrFile:
        chrMap = json.load(chrFile)
    with open('/opt/magstripe/data/chrShiftMap.json') as chrShiftFile:
                chrShiftMap = json.load(chrShiftFile)
    """
    while True:
        time.sleep(.1)
        print 'Swipe Card:'
